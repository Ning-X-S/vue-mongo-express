...
DatePicker.prototype = {
  ...
  FillData: function () {
    ...
    var pickerList = '<div class="zx_select showPicker">' +
    ...
    if (self.options.type > 0) {
      pickerList += '<div class="ub-f1 picker-wrapper" id="dp-wrapper1"><ul></ul></div>'
    }

    // 3级选择
    if (self.options.type > 2) {
      pickerList += '<div class="ub-f1 picker-wrapper" id="dp-wrapper2"><ul></ul></div>'
    }

    pickerList += '<div class="sel_top"></div><div class="sel_bottom"></div>'
    pickerList += '<div class="sel_middle"></div></div></div>'
    modalParent.innerHTML = pickerList
  ...
  setDefaultItem: function (index, dateValue) {
    ...
    self.textArray[index] = nowItem
    console.log(dateValue, nowItem)
  },

  // 获取年列表
  getYearList: function (index, defaultValue) {
    var self = this
    var list = '<li></li><li></li>'; var defaultNum = ''

    var maxYear = self.options.maxYear
    var minYear = self.options.minYear
    if (defaultValue) {
      var num = 0; var isMatch = false
      for (var i = maxYear; i > minYear; i--) {
        if (Number(defaultValue) === i) {
  ...
  getMonthList: function (index, defaultValue) {
    ...
    if (defaultValue) {
      ...
      for (let i = 1; i <= 12; i++) {
        let count = i < 10 ? prefix + i : i
        list += '<li data-value="' + count + '">' + count + unit + '</li>'
      }
      self.setDefaultItem(1, prefix + 1)
    }
